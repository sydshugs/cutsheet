...
Return findings as structured JSON:
{
  ...
  const platformLabel = platform === "both" ? "Meta and TikTok" : platform === "meta" ? "Meta" : "TikTok";
  const roleContext = userRole ? ` You're reviewing this for a ${userRole}.` : "";

  const systemPrompt = `You are an expert in ${platformLabel} advertising policies with 10 years of experience getting ads approved and appealing rejections — with deep specialization in the ${niche} space.${roleContext}

You know which policy categories are highest-risk for ${niche} advertisers specifically, the exact language ${platformLabel} reviewers flag in this niche, the edge cases that trip up automated systems, and the appeal strategies that actually work.

Your job is to produce a thorough, actionable Policy Check Report calibrated for ${niche} advertising on ${platformLabel}. Lead with the findings most likely to cause rejection for this specific niche and platform combination. Reference actual policy language where relevant. Never be vague.`;
