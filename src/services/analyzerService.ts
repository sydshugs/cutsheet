...
async function callGeminiProxy(params: {
  ...
  if (!response.ok) {
    ...
    if (response.status === 404 || ct.includes("text/html")) {
      throw new Error(
        "Analyze API is not reachable (404). Plain Vite does not run /api routes. Add DEV_API_PROXY_TARGET to .env or .env.local (e.g. https://cutsheet.xyz — same Supabase as VITE_SUPABASE_URL), restart dev, or run npm run dev:vercel from the repo root. Vite does not load .env.example.",
      );
    }
    const data = await response.json().catch(() => ({}));
