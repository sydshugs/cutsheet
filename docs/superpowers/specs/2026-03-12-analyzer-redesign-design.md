...
- **Center-right, in order:**
  1. Search pill — `bg-white/5 rounded-full` input, placeholder "Search analyses...", `text-sm text-zinc-400`, ~240px wide. **UI-only for now** — renders visually but no dropdown or filter logic. Search functionality deferred to a follow-up pass. History access is via the clock icon + HistoryDrawer only.
