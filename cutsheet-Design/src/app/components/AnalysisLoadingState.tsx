...
export default function AnalysisLoadingState({ onComplete }: AnalysisLoadingStateProps) {
  ...
  useEffect(() => {
    ...
    const advance = (step: number) => {
      ...
      if (step < 4) {
        ...
        // Last step — call onComplete after its display delay
  ...
  const getDimensionState = (dimIndex: number) => {
    // Map currentStep to specific dimension indices:
    // Dim 0 (Hook) -> Active Step 0. Complete Step >= 1
    // Dim 1 (Clarity) -> Active Step 1. Complete Step >= 2
    // Dim 2 (CTA) -> Active Step 2. Complete Step >= 3
    // Dim 3 (Production) -> Active Step 3 & 4. Complete Step >= 5
    if (dimIndex === 0) {
              ...
              return (
                <div key={name} className="flex flex-col gap-2.5">
                  <div className="flex items-center gap-2">
                    <span className={`text-[14px] font-medium transition-colors duration-500 ${
                  ...
                  <div className="h-1 w-full bg-white/[0.04] rounded-full overflow-hidden shrink-0">
                    <div 
                      className={`h-full rounded-full transition-all ease-out ${
                        status === 'complete' ? 'bg-[#10b981] duration-[600ms] w-full' : 
                        status === 'progress' ? 'bg-[#6366f1] duration-[2000ms] w-[65%]' : 
