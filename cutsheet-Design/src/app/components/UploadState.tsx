...
export default function UploadState({ onFileSelect }: UploadStateProps) {
  return (
    <div className="relative flex flex-col items-center w-full min-h-full font-['Geist',sans-serif]">
      {/* Radial glow — indigo */}
      <div
        className="pointer-events-none absolute inset-0"
        style={{ background: "radial-gradient(ellipse 60% 50% at 50% 40%, rgba(99,102,241,0.08) 0%, transparent 70%)" }}
      />

      <div className="relative flex flex-col items-center w-full max-w-[760px] mx-auto pt-16 pb-20">
        {/* Page Icon Tile — 76×76, rounded-2xl */}
        <div className="w-[76px] h-[76px] rounded-2xl bg-[#6366f1]/[0.08] border border-[#6366f1]/20 flex items-center justify-center mb-6">
          <Zap size={28} color="#6366f1" strokeWidth={1.5} />
        </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 mb-10">
          {["Score any format", "Platform benchmarks", "Priority fix list"].map((label) => (
            <span
              key={label}
              className="rounded-full border px-3 py-1 text-xs text-indigo-400"
        ...
        >
          ...
          <span className="text-[13px] text-[#818cf8] mb-6">
            or browse files
          </span>
